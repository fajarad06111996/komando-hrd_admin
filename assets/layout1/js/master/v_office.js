...
$(document).ready(function() {
    ...
    //     var id = $(this).attr('id');
    //     window.location.replace(dp.link+'/formInfo/'+id);
    // });
    $('#example1').on('click', '.bInfo', function() {
        var id = $(this).attr('id');
        start();
    ...
    if(dp.permC==1){
        ...
        $('#stateForm').submit(async function(e) {
            ...
            // var valid = $('#stateForm').data('validator').form();
            // if(valid==false){
            //     return;
            // }
            try {
        ...
        $('#example1').on('click', '.bEdit', function() {
            var id = $(this).attr('id');
            start();
            // $('#insert_form').html('<form action="<?//= $link; ?>/kaluman" name="vote" method="post" style="display:none;"><input type="text" name="id_edit" value="' + id + '" /></form>');
