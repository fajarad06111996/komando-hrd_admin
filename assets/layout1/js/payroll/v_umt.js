    ...
    $(document).ready(function() {
        if(dp.lock==1){
            $('#btnAdd').click(function() {
                $('#mForm').find('.modal-title').text('Tarik Data UMT');
                $('#myForm').attr('action', dp.link+'/selectUmt');
        ...
        if(dp.permW==1){
            $('#example2').on('click', '.bPublish', function() {
                ...
                $('#publishModal').find('.infoPublish').html('Nomor UMT : '+data+', Publish this UMT?');
                $('#btnPublish').unbind().click(async function() {
                    try {
                        submit = await submitForm('get', dp.link+'/publishUmt', {
                            id: id,
                            all_id: all_id
                        });
            ...
            $('#example2').on('click', '.bEdit', function() {
                var id = $(this).attr('id');
                start();
                window.location.replace(dp.link+'/selectClient/'+id);
            });
        }
        ...
        if(dp.permC==1){
            ...
            $('#example2').on('click', '.bEdit', function() {
                var id = $(this).attr('id');
                start();
                window.location.replace(dp.link+'/selectUmt/'+id);
            });
        }
