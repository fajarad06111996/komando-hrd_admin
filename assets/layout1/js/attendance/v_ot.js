...
var notif, submit, dataPresence, timeArray;
$(document).ready(function() {
    ...
    $('#ot_table').on('click', '.bConfirm', function(){
        var att_idx     = $(this).attr('id');
        var ch_out      = $(this).attr('cout');
        var ot_hour     = $(this).attr('ot-hour');
        var ch_out_date = $(this).attr('cout-date');
        var ch_shift    = $(this).attr('ch-shift');
        var emp_id      = $(this).attr('empid');
        var emp_name    = $(this).attr('empname');
        var ot_id       = $(this).attr('mot');
        var piket       = $(this).attr('piket');
        var countpiket  = $(this).attr('countpiket');
        $('#myConfirmOt').find('.warning-ot').text('');
        $('#mConfirmOt').find('.modal-title').text('Konfirmasi Lembur');
        $('#myConfirmOt').attr('action', dp.link+'/confirmOt');
        $('#myConfirmOt').find('input[name=att_idx]').val(att_idx);
        $('#myConfirmOt').find('input[name=ot_hour]').val(ot_hour);
        $('#myConfirmOt').find('input[name=ch_out_date]').val(ch_out_date);
        $('#myConfirmOt').find('input[name=ch_out_old]').val(ch_out);
        $('#myConfirmOt').find('input[name=ch_shift]').val(ch_shift);
        $('#myConfirmOt').find('input[name=emp_id]').val(emp_id);
        $('#myConfirmOt').find('input[name=ot_id]').val(ot_id);
        $('#myConfirmOt').find('input[name=piket]').val(piket);
        if(piket==1){
            $('#myConfirmOt').find('textarea[name=overtime_reason]').text('Lembur Piket');
        }else{
            $('#myConfirmOt').find('textarea[name=overtime_reason]').text('Lembur Kerja');
        }
        if(countpiket >= 2){
            $('#myConfirmOt').find('.warning-ot').html('Total piket '+emp_name+'(<span class="text-info">'+countpiket+'</span>), Jatah piket sudah habis bulan ini.');
        }
        $("#myConfirmOt").data('validator').resetForm();
        $('#myConfirmOt')[0].reset();
        // $(".select-search").select2().val().trigger('change.select2');
        $(this).tooltip('hide');
        modalDragShow('#mConfirmOt');
        $('#myConfirmOt').find('input[name=check_out]').val(ch_out);
    });
    $('#myConfirmOt').submit(async function(e){
        e.preventDefault();
        spinnersdark(this);
        var url = $(this).attr('action');
        var data = $(this).serialize();
    ...
    var selectajax = $('.js-data-example-ajax').select2({
        ajax: {
