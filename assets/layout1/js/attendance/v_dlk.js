...
$(document).ready(function() {
    ...
    $('#detail_dlk_modal').on('click','#btnAdd', function(){
        var dlkid = $(this).attr('dlkid');
        var dlkdate = $(this).attr('dlkdate');
        $('#dlkForm').data('validator').resetForm();
        $('#dlkForm')[0].reset();
        $('#add_detail_dlk_modal').find('.modal-title').text('Add Detail DLK '+dlkdate);
        $('#dlkForm').find('#img_photo').attr('src', dp.base_url+'assets/media/photos/foto_uploadx.png');
        $('#dlkForm').find('input[name=dlk_idx]').val(dlkid);
        $('#dlkForm').find('input[name=dlk_date]').val(dlkdate);
        $('#dlkForm').attr('action', dp.link+"/add_dlk_detail");
        modalDragShow('#add_detail_dlk_modal');
    });

    $('#detail_dlk_modal').on('click','.bEdit',async function(){
        var id = $(this).attr('id');
        var dlkid = $(this).attr('dlkid');
        var dlkdate = $(this).attr('dlkdate');
        $('#dlkForm').find('#img_photo').attr('src', '');
        $(this).tooltip('hide');
        try {
            submit = await submitForm('get', dp.link+'/get_dlk_detail', {
                id: id
            });
            if(submit==null){
                ...
                $('#dlkForm').data('validator').resetForm();
                $('#dlkForm')[0].reset();
                $('#add_detail_dlk_modal').find('.modal-title').text('Edit Detail DLK '+dlkdate);
                // $('#dlkForm').find('input[name=dlk_idx]').val(dlkid);
                // $('#dlkForm').find('input[name=dlk_date]').val(submit.dlk_date);
                if(submit.url_prof == null || submit.url_prof == ''){
                    $('#dlkForm').find('#img_photo').attr('src', dp.base_url+'assets/media/photos/foto_uploadx.png');
                }else{
                    $('#dlkForm').find('#img_photo').attr('src', submit.url_prof);
                }
                $('#dlkForm').find('input[name=value]').val(submit.value);
                $('#dlkForm').find('textarea[name=description]').val(submit.description);
                $('#dlkForm').find("select[name=employee_id]").select2({width: '100%'}).val(submit.employee_id_en).trigger('change');
                $('#dlkForm').attr('action', dp.link+"/update_dlk_detail/"+id);
                modalDragShow('#add_detail_dlk_modal');
            }
        } catch (error) {
            console.log(error);
        ...
        // $('#dlkForm').data('validator').resetForm();
        // $('#dlkForm')[0].reset();
        // $('#add_detail_dlk_modal').find('.tTgl').text(dlkdate);
        // $('#dlkForm').find('input[name=dlk_idx]').val(dlkid);
        // $('#dlkForm').attr('action', dp.link+"/adit_dlk_detail");
        // modalDragShow('#add_detail_dlk_modal');
    });

    $('#tbDLK').on('click', '.bPublish', function() {
        spinnerdarkDT($('#tbDLK'));
        var id = $(this).attr('id');
        ...
        $('#xStatement').find('.tContent').text('Konfirmasi DLK,');
    ...
    $('#detail_dlk_modal').on('click', '.bSbmit', function() {
        spinnerdarkDT($('#tbDLK'));
        var id = $(this).attr('incid');
        $('#xStatement').modal('show');
        $('#xStatement').find('.modal-title').text('Konfirmasi');
        $('#xStatement').find('.tBtn').text('Konfirmasi ?');
        $('#xStatement').find('.tContent').text('Konfirmasi Semua DLK');
        // $('#xStatement').find('.tName').text(data);
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        select:async function(info) {
            ...
            try {
                submit = await submitForm('GET', dp.link+"/checkDlkByDate", {
                    now: info.startStr
                });
                if(submit.status==true){
                    $('#dlkHForm')[0].reset();
                    $('#dlkHForm').data('validator').resetForm();
                    $('#dlkHForm').find('input[name=dlk_date]').val(info.startStr);
                    $('#dlkHForm').attr('action', dp.link+"/save_dlk_header");
                    modalDragShow('#header_dlk_modal');
                }else{
        ...
        events: async function(info, successCallback, failureCallback){
            ...
            try {
                submit = await submitForm('POST', dp.link+'/getdlk', {
        ...
        eventClick: async function (event) {
            ...
            try {
                const editEventSubmit = await submitForm('get', dp.link+"/get_dlk", {id: id});
                ...
                $('#dlk_date').val(dateEv2);
                $('#detail_dlk_modal').find('.bSbmit').attr('dlkid', id);
                $('#detail_dlk_modal').find('#btnAdd').attr('dlkid', id);
                $('#detail_dlk_modal').find('#btnAdd').attr('dlkdate', dateInc);
                // $('#event_start_date').val(editEventSubmit.start);
                // $('#event_end_date').val(editEventSubmit.end);
                $('#detail_dlk_modal').find('.tDLK').text(dateInc);
                dataTable.ajax.reload();
                modalDragShow('#detail_dlk_modal');
            } catch (error) {
                console.log(error);
            }
        },
    ...
    $('#dlkHForm').submit(async function(e){
        e.preventDefault();
    ...
    $('#dlkForm').submit(async function(e){
        e.preventDefault();
        var validator = $(this).data('validator').form();
        // console.log(validator);
        if(validator == false){
        ...
        try {
            ...
            var data = $(this).serialize();
            submit =  await submitForm('post', url, data);
            $('#add_detail_dlk_modal').modal('hide');
            if(submit.status == true)
            {
                notif = await notifsukses(submit.msg);
                if(notif.value==true){
    ...
    var dataTable = $('#tbDLK').DataTable({
        processing: true,
        serverSide: true,
        scrollx: true,
        responsive: false,
        ajax: {
            url: dp.link+"/get_tb_dlk",
            type: "POST",
            data: function(data) {
                var dlkid2 = $('#detail_dlk_modal').find('#btnAdd').attr('dlkid');
    ...
    $('#dlkForm').on('focus','.moneyx',function(){
        $(this).maskMoney({
...
async function imgKaryawan() {
    var temp_image  = $('#dlkForm').find('input[name=temp_image]').val();
    var url_image   = $('#dlkForm').find('input[name=url_image]').val();
    if(url_image==''){
        if(temp_image==1){
