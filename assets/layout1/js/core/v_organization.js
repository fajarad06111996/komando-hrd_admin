...
$(document).ready(function() {
    $('#btnAdd').click(function() {
        $('#mForm').modal('show');
        $('#btnAdd').tooltip('hide');
        $('#mForm').find('.modal-title').text('Tambah Organisasi');
        $('#myForm').attr('action', dp.link+'/addOrganization');
    ...
    if(dp.permC==1){
        $('#example1').on('click', '.bStatus', function() {
            ...
            $('#xStatement').find('.modal-title').text('Change Status ORGANISASI');
            if(code == 1){
        ...
        $('#example1').on('click', '.bEdit',async function() {
            ...
            $(this).tooltip('hide');
            try {
                submit = await submitForm('get', dp.link+'/editDepartment',{id: id});
                resetRadio();
                $("#myForm").data('validator').resetForm();
                $('#myForm')[0].reset();
                $('#mForm').find('input[name=department_code]').val(submit.department_code);
                $('#mForm').find('input[name=department_name]').val(submit.department_name);
                $('#mForm').find('textarea[name=description]').val(submit.description);
                $('#myForm').attr('action', dp.link+'/updateOrganization/'+id);
                $("select[name=dept_head_idx]").select2({width: '100%'}).val(submit.dept_head_enidx).trigger('change.select2');
                $('#mForm').modal('show');
                if(submit.status==1){
                    $('#xTrue').prop('checked', true).uniform();
                }else{
...
var dataTable = $('#example1').DataTable({ 
    ...
    columnDefs: [
        {targets: [0,1,2,3,4,5,-1], orderable: false,},
