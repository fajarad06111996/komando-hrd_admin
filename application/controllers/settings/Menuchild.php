...
class Menuchild extends AUTH_Controller
{
  ...
  public function index()
  {
    ...
    if(!empty($this->security_function->permissions($this->filename . "-w"))){
      $data['write']  = '<a href="javascript:void(0)" id="btnAdd" class="pull-right text-white small" title="Add New Sub Modul" data-placement="right" data-popup="tooltip"><i class="icon-plus-circle2"></i></a>';
      $data['lock']   = 1;
    }else{
      $data['write']  = '<i class="icon-lock pull-right pt-1 " title="Locked New Sub Modul" data-placement="right" data-popup="tooltip"></i>';
      $data['lock']   = 0;
    }
    $where = array(
      'menu_parent_sub' => 1
    ...
    $data['result']     = $this->mMenu->getSubModul()->result();
    $data['parent']     = $this->mAccess->getData('app_menu','menu_alias','ASC','',$where)->result();
    $this->template->views('pengaturan/v_menuchild', $data);
  }
  ...
  function get_ajax() {
    ...
    // }else{
    // }
    $list = $this->mMenu->get_datatables(2);
    $data = array();
    $no = @$_POST['start'];
    ...
    $output = array(
      "draw"            => @$_POST['draw'],
      "recordsTotal"    => $this->mMenu->count_all(2),
      "recordsFiltered" => $this->mMenu->count_filtered(2),
      "data"            => $data,
    );
  ...
  public function updateMenuChild($id="")
  {
    $msg = array('error' => false);
    $this->form_validation->set_rules('tMenu', 'Nama Menu', 'trim|required');
    $this->form_validation->set_rules('tSubModul', 'Nama Sub Modul', 'trim|required');
  ...
  public function deleteMenuParents($id)
  {
