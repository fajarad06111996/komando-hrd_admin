		...
		<div class="modal-content">
			...
			<form action="" id="myForm" class="form-horizontal form-validasi" method="post">
				<div class="modal-body">
					...
					<div class="form-group row mb-1">
						<label for="inputEmail" class="col-form-label col-sm-3 text-uppercase">Media Icon</label>
						<div class="col-md-9">
...
<div class="panel panel-blue">
	<div class="panel-heading">List Sub<sup class="mt-2">2</sup>Modul
	<?= $write; ?>
  </div>
			...
			<table id="example1" class="table table-striped table-bordered table-hover table-sm1">
				<thead>
					<tr>
						...
						<th>SUB MODUL</th>
						<th>MENU SUB<sup class="mt-2">2</sup>MODUL</th>
						<th>SUB<sup class="mt-2">2</sup> ALIAS</th>
						<th>MENU ACCESS</th>
						<th>URUTAN</th>
						<th>MEDIA ICON</th>
...
<script type="text/javascript">
	...
	<?php if($lock==1): ?>
		$('#btnAdd').click(function() {
			...
			$('#myForm').attr('action', '<?= $link;?>/addmenusubchild');
	...
	<?php if(!empty($this->security_function->permissions($filename . "-c"))): ?>
      ...
      $('.bEdit').tooltip('hide');
			...
			$.ajax({
				...
				success: function(data) {
					if(data===false){
						...
						$('#myForm').attr('action', '<?= $link;?>/updatemenusubchild/'+id);
						$("#myForm").data('validator').resetForm();
					...
					success: function(response) {
						...
						if(response.success) {
							...
							// setTimeout(function () { location.reload(1); }, 2000);
              				...
              				notiferror('Error Delete Modul To Database');
						}
					},
					error: function() {
						$('#deleteModal').modal('hide');
		...
		var dataTable = $('#example1').DataTable({ 
			...
			// 	{"className": "text-center", "targets":[0,6,-1]}
			// ],
