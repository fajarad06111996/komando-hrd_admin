...
class M_dlk extends CI_Model 
{
    ...
    private function _get_datatables_query($idx) {
        $select_column = array(
            'a.*',
            '(SELECT dlk_date FROM dlk_header WHERE idx = a.dlk_idx) as dlk_date',
        ...
        $this->db->from('dlk_detail a');
        $this->db->where('a.dlk_idx', $idx);
        $i = 0;
    ...
    public function getCalendarDlk($month, $year)
    {
        $this->db->select('idx,dlk_code,dlk_date,start_date,end_date')
        ->from('dlk_header')
        ->where(['company_idx' => $this->office, "MONTH(dlk_date) = $month" => null, "YEAR(dlk_date) = $year" => null]);
        // $result = $this->db->get_compiled_select();
        $result = $this->db->get()->result();
        return $result;
    }

    public function checkDlkByDate($dateCalendar)
    {
        $this->db->select('idx,dlk_code,dlk_date,start_date,end_date')
        ->from('dlk_header')
        ->where(['company_idx' => $this->office, "dlk_date" => $dateCalendar]);
        // $result = $this->db->get_compiled_select();
        $result = $this->db->get()->num_rows();
        return $result;
    }

    public function getDlk($idx)
    {
        return $this->db->get_where('dlk_header', ['idx' => $idx, 'company_idx' => $this->office])->row_array();
    }
    public function cekExist($employee_id, $dlk_date)
    {
        $this->db->select('a.*,(SELECT employee_name FROM master_employee WHERE employee_id = a.employee_id) as employee_name')
        ->from('dlk_detail a')
        ->where(['a.employee_id' => $employee_id, 'a.dlk_date' => $dlk_date]);
        $result = $this->db->get()->result();
        return $result;
    }
    public function getDlkDetail($idx)
    {
        return $this->db->get_where('dlk_detail', ['idx' => $idx])->row_array();
    }
    public function insertEvent($data)
    {
        $this->db->trans_begin();
        $this->db->insert('dlk_header', $data);
    ...
    {
        $this->db->trans_begin();
        $this->db->insert('dlk_detail', $dataInsert);
        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();
            return false;
    ...
    public function updateIncDetail($data, $id)
    {
        ...
        $this->db->update('dlk_detail', $data);
        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();
            return false;
